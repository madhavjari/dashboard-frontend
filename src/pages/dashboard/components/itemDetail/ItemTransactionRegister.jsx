...
export default function ItemTransactionRegister({ transactions, fmtNumber, fmtINR }) {
  ...
  return (
    <section className="surface-card overflow-hidden">
      ...
      <div id="item-transaction-register" className="scroll-mt-12 space-y-3 p-4 md:hidden">{visible.length ? visible.map((transaction, index) => <InvoiceCard key={`${transaction.billNo}-${startIndex + index}`} invoiceNumber={transaction.billNo} date={fmtDateIN(transaction.billDate)} title={transaction.party} subtitle={transaction.itemName} amount={fmtINR(transaction.totalAmount)} status={transaction.paymentStatus}><div className="grid grid-cols-2 gap-3 text-xs"><Value label="Quantity" value={`${fmtNumber(getNumericQuantityForUnit(transaction), 1)} ${getUnitLabel(transaction.per)}`} /><Value label="Type" value={<TransactionTypeBadge code={transaction.code} />} /></div></InvoiceCard>) : <Empty query={query} />}</div>
