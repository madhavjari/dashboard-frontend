...
export default function ItemTransactionRegister({ transactions, fmtNumber, fmtINR }) {
  ...
  return (
    <section className="surface-card overflow-hidden">
      ...
      {filtered.length ? <RegisterPagination page={activePage} totalPages={totalPages} startIndex={startIndex} visibleCount={visible.length} totalCount={filtered.length} itemLabel="transactions" onChange={setCurrentPage} scrollTargetId="item-transaction-register" placement="top" /> : null}
      <div id="item-transaction-register" className="scroll-mt-12 space-y-3 p-4 md:hidden">{visible.length ? visible.map((transaction, index) => <InvoiceCard key={`${transaction.billNo}-${startIndex + index}`} invoiceNumber={transaction.billNo} billDate={transaction.billDate} date={fmtDateIN(transaction.billDate)} title={transaction.party} subtitle={transaction.itemName} quantity={`${fmtNumber(getNumericQuantityForUnit(transaction), 1)} ${getUnitLabel(transaction.per)}`} type={transaction.code} amount={fmtINR(transaction.totalAmount)} status={transaction.paymentStatus} />) : <Empty query={query} />}</div>
      <div className="table-scroll hidden max-h-[42rem] overflow-auto md:block"><table className="w-full min-w-[720px] text-sm"><thead className="sticky top-0 z-10 bg-slate-50"><tr className="border-b border-slate-200 text-left text-[11px] uppercase tracking-wide text-slate-500"><th className="px-5 py-3">Date</th><th className="px-5 py-3">Invoice</th><th className="px-5 py-3">Party</th><th className="px-5 py-3 text-right">Quantity</th><th className="px-5 py-3 text-right">Amount</th><th className="px-5 py-3 text-center">Type</th></tr></thead><tbody>{visible.length ? visible.map((transaction, index) => <tr key={`${transaction.billNo}-${startIndex + index}`} className="border-b border-slate-100 transition hover:bg-teal-50/40"><td className="whitespace-nowrap px-5 py-3 text-slate-600">{fmtDateIN(transaction.billDate)}</td><td className="px-5 py-3 font-mono text-xs text-slate-800">{transaction.billNo}</td><td className="px-5 py-3 font-semibold text-slate-900">{transaction.party}</td><td className="px-5 py-3 text-right font-mono-num text-slate-700">{fmtNumber(getNumericQuantityForUnit(transaction), 1)} {getUnitLabel(transaction.per)}</td><td className="px-5 py-3 text-right font-mono-num font-semibold text-slate-950">{fmtINR(transaction.totalAmount)}</td><td className="px-5 py-3 text-center"><TransactionTypeBadge code={transaction.code} /></td></tr>) : <tr><td colSpan={6}><Empty query={query} /></td></tr>}</tbody></table></div>
      {filtered.length ? <RegisterPagination page={activePage} totalPages={totalPages} startIndex={startIndex} visibleCount={visible.length} totalCount={filtered.length} itemLabel="transactions" onChange={setCurrentPage} scrollTargetId="item-transaction-register" /> : null}
    </section>
  );
}
